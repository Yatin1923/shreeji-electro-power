...
class PolycabComprehensiveExtractor:
    def __init__(self, cable_type_slug: str = "mv-power-cable", cable_display_name: str = "MV Power Product"):
        """
    ...
    def extract_product_details(self, product_url: str) -> Dict[str, str]:
        ...
        try:
            ...
            Name = self.extract_Name(html_content)
            ...
            print(f"🔍 Looking for images for: {Name}")
            image_url = self.extract_image_url(html_content)
            if image_url:
                image_path = self.download_image(Name, image_url)
                if image_path:
            ...
            print(f"📚 Looking for brochures for: {Name}")
            brochure_url = self.extract_brochure_url(html_content, Name)
            if brochure_url:
                brochure_path = self.download_brochure(Name, brochure_url)
                if brochure_path:
    ...
    def extract_Name(self, html_content: str) -> str:
        ...
        return title.strip() if title_match else 'Unknown Product'
    ...
    def extract_features(self, html_content: str) -> str:
        ...
        # Method 1: Look for structured Key_Features section
...
def main():
    ...
    available_cable_types = {
        # Main cable types
        '1': ('lv-power-cable', 'LV Power Product'),
        '2': ('mv-power-cable', 'MV Power Product'),
        '3': ('ehv-power-cable', 'EHV Power Product'),
        '4': ('instrumentation-cable', 'Instrumentation Product'),
        '5': ('communication-data-cable', 'Communication & Data Product'),
        '6': ('renewable-energy', 'Renewable Energy Product'),
        
        # Others subcategories
        '7': ('control-cable', 'Control Product'),
        '8': ('fire-protection-cable', 'Fire Protection Product'),
        '9': ('industrial-cable', 'Industrial Product'),
        '10': ('rubber-cable', 'Rubber Product'),
        '11': ('marine-offshoreonshore-cable', 'Marine & Offshore/Onshore Product'),
        '12': ('high-temperature-cable', 'High Temperature Product'),
        '13': ('defence', 'Defence Product'),
        '14': ('domestic-appliance-and-lighting-cable', 'Domestic Appliance and Lighting Product'),
        '15': ('building-wires', 'Building Wires'),
        '16': ('special-cable', 'Special Product'),
        '17': ('aerial-bunched-cable', 'Aerial Bunched Product'),
    ...
    if not selected_choices:
        print("Invalid choice. Using default MV Power Product.")
        selected_choices = ['2']
